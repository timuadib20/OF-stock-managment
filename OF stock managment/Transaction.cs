...
namespace OF_stock_managment
{
    class Transaction : Supplier
    {
        ...
        {
            TransactionDBLayer tdb = new TransactionDBLayer();
           return  tdb.UpdateTranctionDB(this);
        }
        public string deleteTransaction(int id)
        {
            TransactionDBLayer tdb = new TransactionDBLayer();
          return   tdb.DeleteTranctionDB(id);

        }
        public string totalStock(int id)
        {
            TransactionDBLayer tdb = new TransactionDBLayer();
            string ave = tdb.totalStockDB(id);
            return ave;
        ...
        public DataTable viewTransaction(string period)
        {
            TransactionDBLayer tdb = new TransactionDBLayer();
            DataTable dt = tdb.ViewTransactionDB(period);
            return dt;
        }
        ...
        public DataTable searchByInvoiceNumbe(int invoicenumber)
        {
            TransactionDBLayer tdb = new TransactionDBLayer();
            return tdb.searchByInvoiceNumberDB(invoicenumber);
        }
    }
}
